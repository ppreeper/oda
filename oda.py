...
def gen_ingress(ingress, port_list):
    ...
    port_dict = []
    for port_tuple in port_list:
        _, port, path, pathtype = port_tuple.items()
        port_dict.append(
            {
                "path": path[1],
                "pathType": pathtype[1],
                "backend": {"service": {"name": ingress, "port": {"number": port[1]}}},
            }
        )
    cfg = {
...
def gen_deployment(deployment, image, version, ports, volumes, globalvols):
    """Generate Deployment"""
    port_dict = []
    for port in ports:
...
def restore(backup_files):
    ...
    pod_name = get_pod(project)
    for bfile in files:
        print(pod_name, os.path.basename(bfile))
        subprocess.run(
            [
                "kubectl",
                "exec",
                ...
                "-r",
                "-d",
                os.path.join("/opt/odoo/backups/", os.path.basename(bfile)),
            ],
            check=True,
        )
    return
...
def main():
    ...
    subparsers.add_parser("db", help="Access postgresql")
